...
public class AlignTextView extends TextView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int baseline = (targetRect.bottom + targetRect.top - fontMetrics.bottom - fontMetrics.top) / 2; //下面这行是实现水平居中，drawText对应改为传入targetRect.centerX()
        //问题宽度
        float textPx = getTextSize(); //Textview控件总宽度
        ...
        if (suffixMode == 0) {
            ...
            suffixStr = text.substring(text.length() - 1, text.length()); //后缀宽度
